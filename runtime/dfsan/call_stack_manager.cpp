...
namespace qsym {
  ...
  void CallStackManager::visitCall(uintptr_t pc) {
    call_stack_.push_back(pc);
    computeHash();
  }

  void CallStackManager::visitRet(uintptr_t pc) {
  ...
  void CallStackManager::visitBasicBlock(uintptr_t pc) {
    last_pc_ = pc;
    pending_ = true;
  }
  ...
  void CallStackManager::computeHash() {
    ...
    XXH32_update(&state, call_stack_.data(),
        sizeof(uintptr_t) * call_stack_.size());
